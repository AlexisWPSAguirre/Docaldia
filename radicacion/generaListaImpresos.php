...
//         $db->conn->debug = true;
        ...
        <form name='forma' action='generaListaImpresos.php?<?= session_name() . "=" . session_id() . "&krd=$krd&hora_ini=$hora_ini&hora_fin=$hora_fin&minutos_ini=$minutos_ini&minutos_fin=$minutos_fin&tip_radi=$tip_radi&fecha_busq=$fecha_busq&fecha_busqH=$fecha_busqH&fecha_h=$fechah&dep_sel=$dep_sel&num=$num" ?>' method=post>
            ...
            if ($radiNumero == '') {
                ...
                $campos_vista = array("Fecha Radicado", "No.Radicado", "Destinatario", "Dirección", "Asunto", "Remitente", "Usuario Responsable");
                $campos_width = array(120, 117, 123, 120, 250, 120, 120, 120);
                $campos_tabla = '';
                ...
                while (!$rsMarcar->EOF) {
                    ...
                    $campos_tabla = array("$fecha", "$verrad_sal", "$remitente", "$direccionEnvio", "$asunto", "$rem_destino", "$usuarioResponsable");
                    $btt->campos_tabla = $campos_tabla;
                    $btt->tabla_Cuerpo();
                    //error_reporting(7);
                    ...
                    $htmlTable .= '<TD>' . $direccionEnvio . '</TD>';
