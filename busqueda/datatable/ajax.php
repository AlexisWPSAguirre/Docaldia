<?php
    ...
    switch($_POST['type']){
        ...
        case '1':
            ...
            if(isset($request['form']) && !empty($request['form'])){
                ...
                foreach ($request['form'] as $key => $value) {
                    if(isset($request['form'][$key]) && !empty($request['form'][$key])){
                        ...
                        switch ($key) {
                            case 'radi_nume_radi':
                                $queryRadicados .= " AND  radicado.radi_nume_radi LIKE '%".$value."%'";
                            break;
                            case 'sgd_sexp_parexp1':
                                $queryRadicados .= " AND  sgd_sexp_secexpedientes.sgd_sexp_parexp1 LIKE '%".$value."%'";
                            break;
                            case 'sgd_exp_numero':
                                $queryRadicados .= " AND  sgd_sexp_secexpedientes.sgd_exp_numero LIKE '%".$value."%'";
                            break;
                            case 'ra_asun':
                                $queryRadicados .= " AND  radicado.ra_asun LIKE '%".$value."%'";
                            break;
                            case 'sgd_dir_nomremdes':
                                $queryRadicados .= " AND  sgd_dir_drecciones.sgd_dir_nomremdes LIKE '%".$value."%'";
                            break;
                            case 'sgd_dir_doc':
                                $queryRadicados .= " AND  sgd_dir_drecciones.sgd_dir_doc LIKE '%".$value."%'";
                            break;
                            case 'radi_depe_radi':
                                $queryRadicados .= " AND  radicado.radi_depe_radi = '".$value."'";
                            break;
                            case 'radi_depe_actu':
                                $queryRadicados .= " AND  radicado.radi_depe_actu = '".$value."'";
                            break;
                            case 'sgd_trad_codigo':
                                $queryRadicados .= " AND  RIGHT(radicado.radi_nume_radi,1) = '".$value."'";
                            break;
                            case 'tdoc_codi':
                                $queryRadicados .= " AND  radicado.tdoc_codi = ".$value."";
                            break;
            ...
            /*if(!empty($usua_session->fields)){

                switch ($_SESSION['usua_nivel_consulta']) {
                   # Basico -> radicados en mi tramite
                    case '1': case '2':
                        # $queryRadicados .= " AND radicado.radi_usua_actu =".$usua_session->fields['usua_codi']." AND radicado.radi_depe_actu ='".$usua_session->fields['depe_codi']."'";
                    break;
                    # Intermedio -> radicados en tramite de mi dependencia
                    case '3':
                        # $queryRadicados .= " AND radicado.radi_depe_actu ='".$usua_session->fields['depe_codi']."'";
                    break;
                    # Avanzado -> todos
                    case '4': case '5':
                        # $queryRadicados .= " ";
            ...
            $queryRadicados .= " ORDER BY radi_fech_radi DESC  LIMIT 100";
            ...
            if(isset($rsRadicados)){
                while (!$rsRadicados->EOF) {
                    ...
                    $result[] = [
                        ...
                        'nombre'         => $remitente->fields['nombre'],
                        'documento'      => $remitente->fields['documento'],
            ...
            foreach($result as $value){
                ...
                $tableRadicados .= "<tr>";
                    ...
                    $tableRadicados .= "<td>".$value['ra_asun']."</td>";          // <th>Asunto del Radicado</th>       
            ...
            while (!$rsRadicados->EOF) {
                ...
                if ($rsRadicados->fields['radi_path'] != null && $rsRadicados->fields['radi_path'] != '') {
                    ...
                    $result[] = [
                        ...
                        'anex_nomb_archivo' => $rsRadicados->fields['radi_nume_radi'] . '.pdf',
                        'link' => $link,
        ...
        # Inventraio Documnetal -- Detalles -- 
        case '7':
