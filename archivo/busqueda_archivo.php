...
<!--  <td class=titulosError  width="80%" colspan="4" align="center">BUSQUEDA AVANZADA (SOLO PARA RADICADOS ARCHIVADOS) </td>-->
 	...
 	sgd_sbrd_subserierd su where m.sgd_srd_codigo = '$codserie' and su.sgd_srd_codigo = '$codserie'
			...
			and su.sgd_sbrd_fechfin order by detalle ";
                        ...
                        <td width="20%" class='listado2'><input id="buscar_rad" type=text name=buscar_rad value='<?= trim($buscar_rad, ' ') ?>' class="tex_area" title="Buscar por número de radicado">
                        </td></tr>
...
//modificado skina conversion de variable
                            $query = "select SGD_EIT_NOMBRE,SGD_EIT_CODIGO from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as char(5)) LIKE '$exp_edificio'";
...
//modificado skina conversion de variables
                            $sql = "select SGD_EIT_NOMBRE,SGD_EIT_CODIGO from SGD_EIT_ITEMS where cast(SGD_EIT_COD_PADRE as char(5)) LIKE '$buscar_piso'";
                            $rs = $db->query($sql);
...
//                                                echo ' *************** '.$sql;
                                                ...
                                                while (!$rs->EOF) {
                                                    ...
                                                    $qpri = $db->conn->Execute("select sgd_eit_cod_padre from sgd_eit_items where cast(sgd_eit_codigo as char(15)) like '$bus'");
                                                    if (!$qpri->EOF) {
                                                        $it1 = $qpri->fields['SGD_EIT_COD_PADRE'];
                                                        $qsec = $db->conn->Execute("select sgd_eit_cod_padre from sgd_eit_items where cast(sgd_eit_codigo as char(15)) like '$it1'");
                                                        if (!$qsec->EOF) {
                                                            $it2 = $qsec->fields['SGD_EIT_COD_PADRE'];
                                                            $qtir = $db->conn->Execute("select sgd_eit_cod_padre from sgd_eit_items where cast(sgd_eit_codigo as char(15)) like '$it2'");
                                                            if (!$qtir->EOF) {
                                                                $it3 = $qtir->fields['SGD_EIT_COD_PADRE'];
                                                                $qcua = $db->conn->Execute("select sgd_eit_cod_padre from sgd_eit_items where cast(sgd_eit_codigo as char(15)) like '$it3'");
                                                                if (!$qcua->EOF) {
                                                                    $it4 = $qcua->fields['SGD_EIT_COD_PADRE'];
                                                                    $qqin = $db->conn->Execute("select sgd_eit_cod_padre from sgd_eit_items where cast(sgd_eit_codigo as char(15)) like '$it4'");
                                                                    if (!$qqin->EOF) {
                                                                        $it5 = $qqin->fields['SGD_EIT_COD_PADRE'];
                                                                        $qsex = $db->conn->Execute("select sgd_eit_cod_padre from sgd_eit_items where cast(sgd_eit_codigo as char(15)) like '$it5'");
                                                                        if (!$qsex->EOF) {
                                                                            $it6 = $qsex->fields['SGD_EIT_COD_PADRE'];
                                                                            $qset = $db->conn->Execute("select sgd_eit_cod_padre from sgd_eit_items where cast(sgd_eit_codigo as char(15)) like '$it6'");
                                                                            if (!$qset->EOF) {
                                                                                $it7 = $qset->fields['SGD_EIT_COD_PADRE'];
                                                                            }
                                                                        }
